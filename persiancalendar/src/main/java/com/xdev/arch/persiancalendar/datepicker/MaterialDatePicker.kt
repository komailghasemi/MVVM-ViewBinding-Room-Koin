...
class MaterialDatePicker<S>(private val typeface: Typeface?) : DialogFragment() {
    ...
    class Builder<S> private constructor(val dateSelector: DateSelector<S>) {
        ...
        fun setTypeFace(typeface: Typeface): Builder<S> {
    ...
    companion object {
        ...
        fun <S> newInstance(options: Builder<S>): MaterialDatePicker<S> {
            val materialDatePickerDialogFragment =
                MaterialDatePicker<S>(options.typeface)
            val args = Bundle()
            args.putInt(
                OVERRIDE_THEME_RES_ID,
