 ...
 */
class MaterialCalendar<S>(private val typeface: Typeface?) : PickerFragment<S>() {
    ...
    override fun onCreateView(
        ...
        daysHeader.adapter = DaysOfWeekAdapter().apply {
            if (typeface != null) {
        ...
        recyclerView.adapter = monthsPagerAdapter.apply {
            if (typeface != null) {
                setTypeface(typeface)
            }
        }
        ...
        yearSelector?.let { yearSelector ->
            ...
            yearSelector.adapter = YearGridAdapter(this).apply {
                if (typeface != null) {
                    setTypeface(typeface)
                }
            }
        ...
        if (typeface != null) {
            (root as ViewGroup).setFontsRaw(typeface)
        }
        return root
    }
